...
from src.douban_movie_client import DoubanMovieClient
...
douban_movie_client = DoubanMovieClient() # 创建 Douban 客户端实例
...
def generate_douban_topic(model_type, model_name):
    config.llm_model_type = model_type
    ...
    markdown_file_path = douban_movie_client.export_top_stories()
    report, report_file_path = report_generator.generate_hn_topic_report(markdown_file_path)
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("豆瓣TOP电影"):
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_douban_topic, inputs=[model_type, model_name,], outputs=[markdown_output, file_output])
